...
const getcardsComponent = (block) => {
  ...
  const subBlocks = nissancardsWrapper.querySelectorAll('.sub-block');

  for (let index = 0; index < subBlocks.length; index += 1) {
    const element = subBlocks[index];
    const overlay = createElement('div', ['sub-overlay']);
